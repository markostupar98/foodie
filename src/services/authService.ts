...
export const signupDriver = async (driverData) => {
  try {
    const response = await axios.post('http://192.168.1.224:3000/api/auth/signup/driver', driverData);
...
export const signinDriver = async (email, password) => {
  try {
    const response = await axios.post('http://192.168.1.224:3000/api/auth/signin/driver', { email, password });
...
export const signInUser = async (email, password) => {
  try {
    ...
    if (response.data) {
      return { token: response.data.token, userId: response.data.userId, error: null };
    } else {
